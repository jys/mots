...
# Avec des buttons ou lieu des <a> 
import sys
from os import path
import socket
import urllib
...
class ServeurHtml():
    def __init__(self, scriptsDir, local):
        ...
        while True:
            try:
                ...
                # choix du corpus
                #    POST /nouveauCorpus HTTP/1.1
                #    ...
                #    nouveauCorpus=Madame Bovary
                if 'POST /nouveauCorpus HTTP/1.1' in donnejesRecues:
                    m = re.search('nouveauCorpus=(.*)', donnejesRecues)
                    if m is not None:
                ...
                # choix du dejbut de mot + lancement calcul
                #     POST /motCliquej HTTP/1.1
                #     ...
                #     motCliquej=bulanteert
                if 'POST /motCliquej HTTP/1.1' in donnejesRecues:
                    m = re.search('motCliquej=(.*)', donnejesRecues)
                    if m is not None:
    ...
    def afficheTout(self, csock):
        ...
        csock.sendall(("""
            ...
            <form action="nouveauCorpus" method="post">
            """).encode('utf-8'))
        alt = True
        for nomCorpus in list(self.tousLesCorpus.keys()):
            ...
            csock.sendall(("""
                <button value="{}" name="nouveauCorpus" class="{}">{}</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                """.format(nomCorpus, mic, nomCorpus)).encode('utf-8'))
        csock.sendall('</form></div>'.encode('utf-8'))
        # si un corpus est dejjah choisi, affiche le champs pour renseigner le dejbut de mot et l'ordre de fabriquer
        if self.corpus != '':
            csock.sendall(("""
                <div id="af1"><table><tr><td>
        ...
        if len(self.motsFabriquejs) != 0:
            csock.sendall('<div id="af2"><form action="motCliquej" method="post">'.encode('utf-8'))
            alt = True
            for mot in self.motsFabriquejs:
                ...
                csock.sendall(("""
                    <button value="{}" name="motCliquej" class="{}">{}</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    """.format(mot, mic, mot)).encode('utf-8'))
                #csock.sendall(("""
                    #<a href="?MOT;{}"><{}>{}</{}></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        csock.sendall('</div></body></html>'.encode('utf-8'))
